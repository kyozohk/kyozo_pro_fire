...
interface SuspenseMembersDisplayProps {
  communityId: string;
  title?: string;
}
...
const SuspenseMembersDisplay: React.FC<SuspenseMembersDisplayProps> = ({
  communityId,
  title = 'Members'
}) => {
  ...
  const allUsers = useSuspenseQuery<UserProfile>(usersQuery);

  // Filter users who are members of the specified community
  const members = allUsers?.filter((user: UserProfile) => 
    user.communityMemberships?.some((membership: any) => membership.community === communityId)
  );

  // Filter members based on search query
  const filteredMembers = members?.filter((member: UserProfile) => {
...
export default SuspenseMembersDisplay;
