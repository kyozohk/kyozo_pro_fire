...
export default function InboxPage() {
  ...
  useEffect(() => {
    ...
    const responseUsers: UserWithMessages[] = Array.from(usersInvolved).map(userId => {
        ...
        const phoneNumber = user.phoneNumber || user.participation?.phoneNumber || user.waNumber || user.phone || 'Unknown Phone';
  ...
  }, [selectedCommunityId, allMessages, allSentMessages, allUsers, communities, selectedUser?.userId]);
          ...
          <>
                ...
                <div className="flex items-start gap-3">
                 ...
                 <Accordion type="single" collapsible className="w-full mt-2">
                    <AccordionItem value="item-1">
                        <AccordionTrigger className="text-xs">View Raw User Data</AccordionTrigger>
                        <AccordionContent>
            ...
            <div className="flex-1 p-4 overflow-y-auto">
              <div className="space-y-4 max-w-3xl mx-auto">
                {fullConversation.map(message => {
                  ...
                  return (
                    <div key={message.id} className={cn("flex items-end gap-2 w-full", isSentByUser ? "justify-start" : "justify-end")}>
                        {isSentByUser && (
                        ...
                        <div className="flex-grow-0">
                             <Accordion type="single" collapsible className="w-full">
                                <AccordionItem value={message.id} className="border-none">
                                    ...
                                    <AccordionContent>
                                        <pre className="mt-2 p-2 bg-muted text-xs rounded-md overflow-auto max-h-60 w-full max-w-md">
                                            {JSON.stringify(message, null, 2)}
                                        </pre>
                                    </AccordionContent>
                                </AccordionItem>
                            </Accordion>
                            <p className={cn("text-xs text-muted-foreground mt-1", isSentByUser ? "text-left" : "text-right")}>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground text-center p-8">
                ...
                <p className="mt-4 text-lg font-semibold">
                    {isLoading ? "Loading..." : selectedCommunityId ? 'No Conversations' : 'Select a Community'}
                </p>
                <p className="text-sm">
